...
print("=" * 80)
print("VERIFICAR PARTIDO 1252 - VIERNES 16:00")
print("=" * 80)

# Obtener info del partido 1252
cur.execute("""
    SELECT 
        p.id_partido,
        p.fecha_hora,
        ...
        tp1.jugador1_id as j1_p1_id,
        tp1.jugador2_id as j2_p1_id,
        pu1.nombre || ' ' || pu1.apellido as j1_p1,
        pu2.nombre || ' ' || pu2.apellido as j2_p1,
        tp2.id as pareja2_id,
        tp2.jugador1_id as j1_p2_id,
        tp2.jugador2_id as j2_p2_id,
...
# Nuevo horario propuesto: Viernes 27/03 16:00
nuevo_horario = datetime(2026, 3, 27, 16, 0)
print(f"\n{'=' * 80}")
print(f"VERIFICAR: Viernes 27/03 16:00")
print(f"{'=' * 80}")
...
for cancha in canchas_torneo:
    # Verificar si hay partidos que se solapen con 16:00-17:10
    cur.execute("""
...
for pareja in parejas_restricciones:
    ...
    # Verificar si 16:00-17:10 está dentro de alguna restricción
    conflicto = False
    for r in restricciones_viernes:
        ...
        # Partido: 16:00-17:10 (16.0 - 17.17)
        partido_inicio = 16.0
        partido_fin = 17.17
    ...
    if not conflicto:
        print(f"   ✅ {nombre_pareja}: Disponible (restricciones no afectan 16:00)")
...
if canchas_disponibles and todos_disponibles:
    print(f"\n✅ SÍ SE PUEDE MOVER A VIERNES 16:00")
