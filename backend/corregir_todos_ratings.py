"""
Script para verificar y corregir ratings de TODOS los usuarios automáticamente
"""
import os
import sys
...
def verificar_y_corregir_todos():
    ...
    # Obtener todos los usuarios con partidos
    query = text("""
    ...
    for user_id, nombre, apellido, rating_actual in usuarios:
        ...
        if not categoria_inicial:
            usuarios_sin_categoria.append(f"{nombre} {apellido}")
            continue
        ...
        if diferencia != 0:
            ...
            query_update = text("""
                ...
                WHERE id_usuario = :user_id
            """)
            session.execute(query_update, {"rating": rating_esperado, "user_id": user_id})
            ...
            print(f"✅ {nombre} {apellido}: {rating_actual} → {rating_esperado} (diferencia: {diferencia:+d})")
        else:
    ...
    if usuarios_corregidos:
        ...
        for u in usuarios_corregidos:
            print(f"  {u['nombre']:<30} {u['categoria']:<15} {u['rating_anterior']:>5} → {u['rating_nuevo']:>5} ({u['diferencia']:+4})")
    ...
    print(f"\n{'='*100}")
    print(f"✅ Proceso completado. {len(usuarios_corregidos)} usuarios corregidos.")
    print(f"{'='*100}\n")
