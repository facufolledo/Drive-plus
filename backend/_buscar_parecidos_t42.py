"""Buscar usuarios temp (T38 y T42) que coincidan con usuarios reales (Firebase).
Usuarios reales = contraseña vacía/null (autenticados por Firebase).
Usuarios temp T38 = email @driveplus.temp
Usuarios temp T42 = email @temp.com
"""
import os, sys
...
with engine.connect() as c:
    ...
    for t, t_nombre, (score, r, r_nombre) in matches:
        # Verificar en qué torneos está el temp
        torneos = c.execute(text("""
            SELECT DISTINCT tp.torneo_id, t.nombre
        ...
        torneo_tags = ", ".join([f"T{tid}" for tid, _ in torneos]) if torneos else ""
        pct = f"{score*100:.0f}%"
        
        print(f"\n  {pct} | TEMP: {t_nombre} (ID={t[0]}, rating={t[5]}, pj={t[6]}) [{torneo_tags}]")
